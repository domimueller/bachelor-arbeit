...
Für die Dokumentation der Lösung verwendet der Autor die UML-Notation. Bei der Modellierung werden Empfehlungen aus dem Buch \flqq{}UML 2 glasklar\frqq{} \cite{Uml-modellierung2012} berücksichtigt (Diagramme siehe nächste Seite). 
...
Nebst der Funktion \texttt{threshold()} und dem Typ \texttt{THRESH_BINARY} bietet OpenCV weiteren Typen wie \texttt{THRESH_BINARY_INV} oder \texttt{THRESH_TRUNC} und das adaptive Schwellwertverfahren \texttt{adaptiveThreshold()} an. Adaptive Schwellwertverfahren ermöglichen den Einsatz spezifischer Schwellwerte für einen Pixel im Zielbild. Dieser spezifische Schwellwert wird auf Basis einer Gruppe benachbarter Pixel  ermittelt \cite[S.342 f]{FernandezVillan2019}. Die im Rahmen der Bachelor-Arbeit entwickelte Lösung unterstützt die Parametrierung dieser Funktionen. Anhand des Value Objects \texttt{\glqq{}AdaptiveThresholdingConfiguration\grqq{}} kann aus der Enumeration \texttt{\glqq{}AdaptiveThresholdingType\grqq{}} das Verfahren zur Schwellwertanalyse ausgewählt und konfiguriert werden. 
